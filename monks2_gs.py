...
print(f"starting grid search - exploring {len(grid2)} configs")
for i, config in enumerate(grid2):
    print(f"{i+1}/{len(grid2)}")
    ...
    for i in range(n_trial):
        network.fit(X_train2, y_train2, X_test2, y_test2)
        accuracy = network.score(X_test2, y_test2, 'accuracy')
...
df_scores.to_csv('/kaggle/working/monks2_gs.csv')