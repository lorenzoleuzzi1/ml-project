...
class Network:
    ...
    def fit(self, X_train, Y_train):
        self.set_fitting(X_train, Y_train)
        ...
        for epoch in range(self.epochs):
            ...
            if self.early_stopping:
                stopping, weights_backtracked, bias_backtracked = self.check_early_stopping(stopping, train_losses, val_losses)

                if stopping <= 0: # stopping criteria satisfied
                    val_losses[-self.stopping_patience:] = []
                    val_scores[-self.stopping_patience:] = []
                    train_losses[-self.stopping_patience:] = [] # REVIEW
